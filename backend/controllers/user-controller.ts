...
export const getRecommendedUsers = async (req: RequestUser, res: Response) => {
    try {
        ...
        if (user?.friends) {
            recommendedUsers = await sql`SELECT * FROM Users WHERE NOT id=${id} AND id NOT IN (${Number(String(user?.friends))}) AND completed_profile=true` as User[];
        } else {
...
export const getFriends = async (req: RequestUser, res: Response) => {
    try {
        ...
        const friends: User[] = await sql`SELECT first_name, last_name, native_language, profile_picture, bio FROM Users WHERE id IN (${String(req.user?.friends)})` as User[];
...
export const acceptFriendRequest = async (req: RequestUser, res: Response) => {
    try {
        ...
        const existingFriendRequest: FriendRequest[] = await sql`SELECT * FROM friendrequests WHERE sender=${recipientId} AND recipient=${accepter} AND status='pending';` as FriendRequest[];
...
export const getFriendRequests = async (req: RequestUser, res: Response) => {
    try {
        ...
        const sentFriendRequests = await sql`SELECT recipient FROM friendrequests WHERE sender=${id}`;

        const incomingFriendRequestProfiles: FriendRequestData[] = [];
        incomingFriendRequests.map(async (id) => {
            const profile = (await sql`SELECT first_name, last_name, profile_picture, native_language FROM Users WHERE id=${id}` as FriendRequestData[])[0];
            incomingFriendRequestProfiles.push(profile);
        })

        const sentFriendRequestProfiles: FriendRequestData[] = [];
        sentFriendRequestProfiles.map(async (id) => {
            const profile = (await sql`SELECT first_name, last_name, profile_picture, native_language FROM Users WHERE id=${id}` as FriendRequestData[])[0];
            sentFriendRequestProfiles.push(profile);
        })
